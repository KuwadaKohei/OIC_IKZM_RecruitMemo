package ikzm_jhm_dao;

import java.sql.Connection;
import java.sql.PreparedStatement;
import java.sql.ResultSet;
import java.sql.SQLException;
import java.util.ArrayList;
import java.util.List;

import ikzm_jhm_dto.ExamContent;

/**
 * 試験内容マスタ(exam_content)を扱うDAO。
 */
public class ExamContentDAO {

	private final ConnectionDAO connectionDao = new ConnectionDAO();

	/**
	 * 全試験内容をID昇順で取得する。
	 * 
	 * @return DTOリスト
	 */
	public List<ExamContent> findAll() throws Exception {
		List<ExamContent> list = new ArrayList<ExamContent>();
		String sql = "SELECT content_id, category, name, needs_detail FROM exam_content ORDER BY content_id";

		try (Connection conn = connectionDao.getConnection(); PreparedStatement stmt = conn.prepareStatement(sql)) {
			ResultSet rs = stmt.executeQuery();
			while (rs.next()) {
				list.add(mapExamContent(rs));
			}
		} catch (SQLException e) {
			throw new Exception("DB-0040");
		}

		return list;
	}

	/**
	 * 主キーで試験内容を取得する。
	 * 
	 * @param contentId コンテンツID
	 * @return DTO（なければnull）
	 */
	public ExamContent findById(int contentId) throws Exception {
		ExamContent content = null;
		String sql = "SELECT content_id, category, name, needs_detail FROM exam_content WHERE content_id = ?";

		try (Connection conn = connectionDao.getConnection(); PreparedStatement stmt = conn.prepareStatement(sql)) {
			stmt.setInt(1, contentId);
			ResultSet rs = stmt.executeQuery();
			if (rs.next()) {
				content = mapExamContent(rs);
			}
		} catch (SQLException e) {
			throw new Exception("DB-0040");
		}

		return content;
	}

	/**
	 * ResultSetをExamContent DTOへ変換する。
	 * 
	 * @param rs クエリ結果
	 * @return DTO
	 * @throws SQLException 変換例外
	 */
	private ExamContent mapExamContent(ResultSet rs) throws SQLException {
		ExamContent content = new ExamContent();
		content.setContentId(rs.getInt("content_id"));
		content.setContentCategory(rs.getString("category"));
		content.setContentName(rs.getString("name"));
		content.setNeedsDetail(rs.getBoolean("needs_detail"));
		return content;
	}
}
